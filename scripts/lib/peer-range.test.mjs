...
describe('Eval compatibility qualification', () => {
  it('keeps the existing development pin while permitting the verified packed consumers', () => {
    ...
    expect(evalCompatibility('0.182.0', '0.183.0')).toEqual({
      version: '0.183.0',
      peerRange: '>=0.182.0 <0.185.0',
    })
    expect(evalCompatibility('0.182.0', '0.184.0')).toEqual({
      version: '0.184.0',
      peerRange: '>=0.182.0 <0.185.0',
    })
  })
  ...
  it('refuses an unqualified development version even with a supported consumer override', () => {
    expect(() => evalCompatibility('0.185.0', '0.183.0')).toThrow(
      'unverified Eval development version',
    )
  })

  it.each(['0.181.9', '0.185.0', '1.0.0', '0.183.0-rc.1', 'latest', ''])
