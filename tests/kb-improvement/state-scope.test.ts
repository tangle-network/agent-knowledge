...
describe('declared candidate state', () => {
  it.skipIf(process.platform !== 'linux')(
    'uses the declared pages directory in the maintained research writer and evaluator',
    async () => {
      await withKb(async (root) => {
        ...
        await promoteKnowledgeCandidate({
          root,
          candidate: knowledgeImprovementCandidateRef(result),
        })
  ...
  it.skipIf(process.platform !== 'linux')(
    'isolates siblings, resumes exact state, and promotes and restores declared research and custom pages',
    async () => {
      await withKb(async (root) => {
        await prepare(root)
